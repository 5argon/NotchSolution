...
- Minimum supported version is now Unity 6.3 LTS (6000.3). This is a **breaking change**.
- Repository restructured into a Unity project layout. The package lives at `src/NotchSolution/Assets/NotchSolution` and is installed via Git URL with `?path=src/NotchSolution/Assets/NotchSolution`.
- Safe area and cutouts are read through `UnityEngine.Device.Screen`, so components respond to Unity's built-in Device Simulator directly, in the editor and in a build.

### Removed

- The custom Notch Simulator window, its device database (`*.device.json`), device mockup overlays, and the `MockupCanvas` scene overlay. Unity's built-in Device Simulator covers these, and no hidden GameObject is added to the scene.
- `INotchSimulatorTarget`, `NotchSolutionUtility.ShouldUseNotchSimulatorValue`, `SimulationDevice`, and the project/preferences settings and shortcuts that only served the old simulator.

### Added

- Components update live in the editor when the Device Simulator changes device or orientation, without dirtying the scene. `SafePadding` and other UI-behaviour components refresh from an `[ExecuteAlways]` edit-mode loop, matching how the adaptation components already behave.

### Fixed

- Replaced the obsolete `AnimationMixerPlayable.Create(..., normalizeWeights)` overload and `Object.FindObjectsOfType` API.
